...
async def check_role(ctx, member: discord.Member = None):
    print(f"🚨 check command called by {ctx.author} at {datetime.utcnow()}")  # <-- აქ ჩასვი

    if member is None:
        await ctx.send("⛔️ შეცდომა : გთხოვთ მიუთითოთ მომხმარებელი, რომლის როლსაც ამოწმებთ. გამოიყენეთ ბრძანება ასე: !check @user.")
        return
